...
class Database():
    def __init__(self, dburi):
        ...
        try:
            ...
            message = sisyphus.automation.utils.formatException(exceptionType, exceptionValue, exceptionTraceback)
            ...
            for attempt in self.max_db_attempts:
                try:
                    ...
                    exceptionType, exceptionValue, exceptionTraceback = sys.exc_info()
                    errorMessage = sisyphus.automation.utils.formatException(exceptionType, exceptionValue, exceptionTraceback)
    ...
    def logMessage(self, s, reconnect = True):
        ...
        id = os.uname()[1]
        s  = sisyphus.automation.utils.makeUnicodeString(s)

        log_doc = {
            "datetime"  : sisyphus.automation.utils.getTimestamp(hiresolution=True),
            "worker_id" : id,
            "message"   : u"%s: %s" % (id, s),
            "type"      : "log"
            }
        ...
        for attempt in attempts:
            try:
                ...
                errorMessage = ('connectToDatabase: %s: %s, database %s not available, exception: %s' %
                                (os.uname()[1], sisyphus.automation.utils.getTimestamp(), self.dburi,
                                 sisyphus.automation.utils.formatException(exceptionType, exceptionValue, exceptionTraceback)))
    ...
    def checkDatabase(self):
        try:
            ...
            self.connectToDatabase(range(1))
            self.logMessage('checkDatabase: exception %s: %s' % (str(exceptionValue), sisyphus.automation.utils.formatException(exceptionType, exceptionValue, exceptionTraceback)))
