...
function userOnAfterPage()
{
  ...
  try
  {
    ...
    var content;
    var meta_list = doc.getElementsByTagName('meta');
    for (var imeta = 0; imeta < meta_list.length; imeta++) {
      var meta = meta_list[imeta];
      if (meta.getAttribute('http-equiv') && (content = meta.getAttribute('content'))) {
        var capturesmeta = r.exec(content);
        metacharset = capturesmeta ? capturesmeta[1] : null;
        if (metacharset)
          break;
      }
    }

    if (metacharset && doc.characterSet.toLowerCase() != metacharset.toLowerCase()) {
      cdump('charset differs: ' + doc.location.href + ' doc.characterSet=' + doc.characterSet + ', metacharset=' + metacharset);
      completePage();
      return;
    }
